...
class FritzBoxGuestWifi(object):
    ...
    def reconnect_fritzbox(self, call):
        _LOGGER.info('Reconnecting @ FRITZ!Box')
        self._connection.reconnect()

    def turn_on(self, call):
        _LOGGER.info('Turning on guest wifi @ FRITZ!Box')
        self._handle_turn_on_off(True)

    def turn_off(self, call):
        _LOGGER.info('Turning off guest wifi @ FRITZ!Box')
    ...
    def _handle_turn_on_off(self, turn_on):
        ...
        try:
            self._connection.call_action('WLANConfiguration:2', 'SetEnable', NewEnable=new_state)
